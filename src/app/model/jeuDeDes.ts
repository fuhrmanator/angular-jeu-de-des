...
import { ResultatLancer } from "./resultatLancer";
...
export class JeuDeDes {
  ...
  public async asyncDemarrerJeu(nom: string): Promise<string> {
  ...
  public jouer(nom: string): ResultatLancer {
    const joueur = this._joueurs.get(nom);
    if (!joueur) {
      throw new Error(`Joueur '${nom}' n'existe pas.`); // TODO erreur précise
    }
    const somme = this.brasser()
    joueur.lancer();
    const gagne = somme === 7;
    if (gagne) joueur.gagner();
    const resultat: ResultatLancer = {
      nom: nom,
      somme: somme,
